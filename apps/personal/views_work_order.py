...
class CostAppUpdateDetailView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if 'id' in request.GET and request.GET['id']:
            ...
            # 获取申请人近十条申请记录
            user_id0 = work_order.cretor_id
            user_orders = WorkOrder.objects.filter(Q(cretor_id=user_id0) & ~Q(id=request.GET.get('id'))).order_by("-create_time")[:10]
            for i in user_orders:
                if len(i.title) >= 10:
                    i.title = i.title[:10] + "..."
            ret['user_orders'] = user_orders
...
class WorkOrderAppOtherDetailView(LoginRequiredMixin, View):
    """
    审批申请 弹窗内点击其他申请显示的详情页
    """
    def get(self, request):
        ret = dict()
